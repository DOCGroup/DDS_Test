...
char *output_file = "client_results";

int
parse_args (int argc, char **argv)
{
  ACE_Get_Opt get_opts (argc, argv, "i:p:s:t:");
  int c;
...
main (int argc, char *argv[])
{
  ...
  ACE_NEW_RETURN (stats_ptr,
                  PubSub_Stats (output_file,
                                primer_samples,
                                stats_samples,
                                seq_len),
                  -1);
  proxy.soap->stats = stats_ptr;
  ACE_Auto_Basic_Ptr<PubSub_Stats> safety (stats_ptr);
  
  for (unsigned long i = 0; i < total_samples; ++i)
    {
      seq_num = (i == total_samples - 1 ? 0 : i + 1);
      
      switch (DATA_TYPE)
        {
          case BYTE_SEQ:
            status = proxy.ns__conversionByteArray (in, seq_num);
            break;
          case COMPLEX:
            status = proxy.ns__conversionNestedArray (complex_in, seq_num);
            break;
          default:
            break;
        }
