...
#define CONDITIONAL_GENERIC_READER_CONSTRUCTION(typeRequested) \
    if (strcmp(typeName, #typeRequested) == 0) { \
        typeRequested##TypeSupportImpl* ts_servant = \
          new typeRequested##TypeSupportImpl; \
        PortableServer::ServantBase_var safe_ts = ts_servant; \
        typeRequested##TypeSupport_var ts_ref = \
          TAO::DCPS::servant_to_reference (ts_servant); \
        typeRequested##DataReader_ptr dataReader; \
        ts_ref->register_type(domainParticipant, typeName); \
