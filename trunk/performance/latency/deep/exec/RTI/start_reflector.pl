...
use Env (PATH);
use Env (DDS_ROOT);
use Env (RTDEEP_SCRIPTS);
use Env (RTDEEP_ROOT);
use lib "$ACE_ROOT/bin";
use PerlACE::Run_Test;
use Cwd;

require  "$RTDEEP_SCRIPTS/scripts.lib";

#chdir("$DBE_ROOT/performance/throughput/NDDS");
system("$RTDEEP_SCRIPTS/enable_cores.sh");
...
&setEnvironment("$RTDEEP_ROOT/settings/environment");
...
$parameters = "";
$parameters .= " scenario.xml ReflectorParticipant $priority";
