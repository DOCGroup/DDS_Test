...
$client_results = PerlACE::LocalFile ("client_results");
$server_results = PerlACE::LocalFile ("server_results");
...
foreach $sample_size (@dataSizes) {
  ...
  $SV->Arguments ("-p $num_primers -i $num_invocations -s $sample_size");

  $SV->Spawn ();

  sleep (1);

  $CL->Arguments ("-t $test_type -p $num_primers -i $num_invocations -s $sample_size");
