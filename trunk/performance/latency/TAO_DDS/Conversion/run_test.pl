...
$status = 0;
$pub_output_file = "pub_results";
$sub_output_file = "sub_results";
...
unlink $pub_output_file;
unlink $sub_output_file;
...
foreach $seq_len (@dataSizes) {
  ...
  $SV->Arguments ("-t $test_type -o $sub_output_file -p $num_primers -i $num_invocations -s $seq_len $use_udp");

  $SV->Spawn ();
  
  sleep (3);

  $CL->Arguments ("-t $test_type -o $pub_output_file -p $num_primers -i $num_invocations -s $seq_len $use_udp");
