...
const char *sub_output = "sub_results";
extern PubSub_Stats *sub_stats;
...
{
  ACE_Get_Opt getopt (argc, argv, ACE_TEXT ("t:i:p:s:u"));
  int c;
...
int main (int argc, char *argv[])
{
  ...
  /// Set global extern pointer in DDSPerfTestC.h. We add 2 to
  /// primer_samples since there is an extra write for initialization,
  /// and an extra write for registration.
  PubSub_Stats stats (sub_output,
                      primer_samples + 2,
                      stats_samples,
                      seq_len);
  sub_stats = &stats;
  ...
  if (useTCP)
  ...
  else
    {
      ...
      writer_config->local_address_.set ("localhost:1236");
      pub_tcp_impl->configure (writer_config.in ());
    }
